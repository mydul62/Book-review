...
const WishList = ({data}) => {
  const {bookName,author,tags,publisher,rating,category,bookId,image,totalPages}=data
  return (
    <div className="md:grid flex flex-col  md:grid-cols-12   gap-6 border border-[#13131326] p-6 rounded-xl">
    <div className="h-[231px] col-span-3 flex justify-center items-center bg-[#1313130D] rounded-xl"><img className="  w-[130px] h-[172px] " src={image}/></div>
    <div className=" col-span-9 space-y-4">
      <h2 className=" text-2xl font-bold text-[#131313CC]  font-Playfair">{bookName} </h2>
...
Year of Publishing: 1924</h2>
     ...
     <div className=" flex gap-6">
      <h2 className=" flex items-center gap-2"><HiOutlineUsers size={20} />Publisher: {publisher}</h2>
      <h2 className=" flex items-center gap-2"><MdOutlineContactPage size={20}/>
Page {totalPages}</h2>
     </div>
     </div>
       <div className=" flex  flex-wrap items-center gap-3">
        <button className=" px-[20px] py-[11px] bg-[#328EFF26] text-[#328EFF] font-sans font-medium rounded-3xl ">Category: {category}</button>
        <button className=" px-[20px] py-[11px] bg-[#FFAC3326] text-[#FFAC33] font-sans font-medium rounded-3xl ">Rating: {rating}</button>
